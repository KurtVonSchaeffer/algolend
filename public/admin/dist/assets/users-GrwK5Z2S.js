import{supabase as m}from"./supabaseClient-DdIec1HK.js";/* empty css              *//* empty css               */import{i as L}from"./layout-Nmoyu__-.js";import{n as B,o as C,j as S,p as D}from"./dataService-CkuKBXml.js";import{v as k,b as T,a as b}from"./utils-CZwHw4kl.js";import"https://esm.sh/@supabase/supabase-js@2";import"./theme-CYs9TE7o.js";async function w(e,t={},a=!0){let{data:{session:n}}=await m.auth.getSession();if(!n){const{data:i}=await m.auth.refreshSession();n=i.session}if(!n)throw window.location.replace("/auth/login.html"),new Error("No session — redirecting to login");const l={"Content-Type":"application/json",...t.headers||{},Authorization:`Bearer ${n.access_token}`},s=await fetch(e,{...t,headers:l});if(s.status===401&&a){const{data:i}=await m.auth.refreshSession();if(!i.session)throw window.location.replace("/auth/login.html"),new Error("Session expired — redirecting to login");return w(e,t,!1)}return s}function U(e,t={}){const{showActions:a=!0,className:n="",isLuhnValid:l=!0}=t;let s=e.full_name||e.first_name+" "+e.surname;s=s.replace("NOT_PROVIDED","").trim();const i=s.split(" ").map(p=>p[0]).join("").substring(0,2).toUpperCase(),o=l?"bg-emerald-50 text-emerald-700 border-emerald-100":"bg-red-50 text-red-700 border-red-100",r=l?"check_circle":"warning",c=l?"Verified ID":"ID Error";return`
    ...
    `}let E=[],g=[],v=null,u=null,d=1;const h=20;let I="all";const A=`
...
`,_=e=>["admin","super_admin","base_admin"].includes(e),M=e=>({super_admin:"SUPER ADMIN",admin:"BRANCH MANAGER",base_admin:"LOAN OFFICER"})[e]||"CLIENT",N=e=>{const t=(e||"UNKNOWN").toUpperCase();let a="bg-gray-100 text-gray-600";return t==="DISBURSED"&&(a="bg-green-100 text-green-700"),t==="DECLINED"&&(a="bg-red-100 text-red-700"),["STARTED","SUBMITTED"].includes(t)&&(a="bg-blue-50 text-blue-700"),`<span class="px-2 py-0.5 rounded text-[10px] font-bold ${a}">${t}</span>`};window.switchView=e=>{const t=document.getElementById("view-list"),a=document.getElementById("view-detail");e==="detail"?(t.classList.add("hidden"),a.classList.remove("hidden")):(t.classList.remove("hidden"),a.classList.add("hidden"),u=null)};const R={super_admin:["Full access to everything","Manage all staff and branches","Change system settings","View all financial reports","Override credit decisions"],admin:["Manage staff in their branch","Approve and decline applications","View branch financials","Transfer clients between branches"],base_admin:["View and process loan applications","Upload and verify documents","Contact clients","Capture payments"]},P={super_admin:"Super Administrator",admin:"Branch Manager",base_admin:"Loan Officer"};function H(e,t){const a=R[e.role]||["Standard access"],n=P[e.role]||"Staff",l=t.find(i=>i.id===e.branch_id)?.name||"No branch assigned",s=a.map(i=>`
    ...
    `}function O(e){const t=e.financials||{},a=e.loans.filter(s=>["DISBURSED","ACTIVE"].includes(s.status)).reduce((s,i)=>s+Number(i.amount),0),n=e.loans.length===0?'<tr><td colspan="5" class="p-12 text-center text-xs font-bold text-slate-300">No applications found.</td></tr>':e.loans.map(s=>`
            <tr class="hover:bg-slate-50 transition-colors cursor-pointer group" onclick="window.location.href='/admin/application-detail?id=${s.id}'">
                ...
                <td class="px-6 py-5 text-sm font-black text-slate-900">${b(s.amount)}</td>
        ...
        <div class="glass-card p-6 rounded-2xl">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="bg-surface-container p-3 rounded-xl">
                    ...
                    <p class="text-sm font-bold text-on-surface">${b(t.monthly_income||0)}</p>
                </div>
                <div class="bg-surface-container p-3 rounded-xl">
                    <p class="text-[10px] text-outline uppercase">Expenses</p>
                    <p class="text-sm font-bold text-on-surface">${b(t.monthly_expenses||0)}</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            <div class="glass-card p-4 rounded-2xl">
                ...
                <div class="text-2xl font-extrabold mt-1" style="color:var(--color-primary)">${b(a)}</div>
            </div>
            <div class="glass-card p-4 rounded-2xl">
    ...
    `}const y={borrower:0,base_admin:1,admin:2,super_admin:3};window.openUserDetail=async e=>{try{document.body.style.cursor="wait";const t=await B(e);if(!t?.profile)throw new Error("Profile not found for this user.");u=t;const a=t.profile,n=k(a?.identity_number||a?.id_number),l=_(a.role),s=document.getElementById("profile-card-container");s&&(s.innerHTML=U(a,{isLuhnValid:n}));const i=document.getElementById("detail-right");i&&(i.innerHTML=l?H(a,g):O(t));const o=document.getElementById("btn-transfer-branch");o&&(o.onclick=()=>window.openBranchModal());const r=document.getElementById("btn-remove-staff");if(r){const c=y[v?.role]||0,p=y[a.role]||0,f=l&&c>p&&a.id!==v?.id;r.classList.toggle("hidden",!f),r.onclick=()=>window.removeStaff(a.id,a.full_name)}window.switchView("detail")}catch(t){console.error("Detail Error:",t?.message||t),alert(`Could not load user details: ${t?.message||"Unknown error — check console"}`)}finally{document.body.style.cursor="default"}};window.removeStaff=async(e,t)=>{if(confirm(`Remove ${t} from the platform? This cannot be undone.`))try{const a=await w(`/api/admin/remove-staff/${e}`,{method:"DELETE"}),n=await a.json();if(!a.ok)throw new Error(n.error||"Failed to remove staff");alert(`${t} has been removed.`),window.switchView("list"),window.location.reload()}catch(a){alert("Could not remove staff: "+a.message)}};window.openBranchModal=()=>{if(!u)return;const e=u.profile;document.getElementById("modal-username").textContent=e.full_name;const t=document.getElementById("modal-branch-select");t.innerHTML='<option value="online">Online / Unassigned</option>',g.forEach(a=>{const n=document.createElement("option");n.value=a.id,n.textContent=a.name,e.branch_id===a.id&&(n.selected=!0),t.appendChild(n)}),document.getElementById("branch-modal").classList.remove("hidden")};window.confirmBranchTransfer=async()=>{const e=document.getElementById("btn-confirm-transfer"),t=document.getElementById("modal-branch-select").value,a=u.profile;try{e.disabled=!0,e.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Moving...';const n=t==="online"||!t?null:parseInt(t,10),{error:l}=await m.from("profiles").update({branch_id:n}).eq("id",a.id);if(l)throw l;await m.from("loan_applications").update({branch_id:n}).eq("user_id",a.id),alert("Success! User transferred."),document.getElementById("branch-modal").classList.add("hidden"),window.location.reload()}catch(n){alert("Transfer failed: "+n.message),e.disabled=!1,e.textContent="Confirm Transfer"}};const V=e=>{const t=document.getElementById("users-table-body");if(!t)return;const a=(d-1)*h,n=e.slice(a,a+h);if(n.length===0){t.innerHTML='<tr><td colspan="5" class="p-20 text-center text-slate-300 font-bold">No results matching your query.</td></tr>';return}t.innerHTML=n.map(s=>{const i=s.branches?.name||"Online",o=k(s.identity_number||s.id_number);return`
